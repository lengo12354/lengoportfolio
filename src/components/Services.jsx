...
const Services = () => {
    return (
        <section id="services" className="py-32  relative overflow-hidden">
            <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[500px] h-[500px] bg-violet-600/10 rounded-full blur-[128px] pointer-events-none" />
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                    {packages.map((pkg, index) => (
                        <motion.div
                            ...
                            className={`relative p-8 rounded-2xl border ${pkg.highlight ? 'border-violet-500 bg-violet-900/10' : 'border-white/10 bg-zinc-900/50'} backdrop-blur-sm flex flex-col`}
                        >
                            {pkg.highlight && (
                                <div className="absolute -top-4 left-1/2 -translate-x-1/2 px-4 py-1 bg-violet-500 text-white text-xs font-bold uppercase tracking-widest rounded-full">
                                    Most Popular
                                </div>
                            )}
                            ...
                            <MagneticButton className={`w-full ${pkg.highlight ? 'bg-violet-600 hover:bg-violet-700 text-white' : 'bg-white/10 hover:bg-white/20 text-white'}`}>
                                Choose {pkg.name}
                            </MagneticButton>
                        </motion.div>
                    ))}
